...
def download_dataset(dataset_url: str, dataset_dir: Path):
    if dataset_dir.is_dir() and len(list(dataset_dir.iterdir())) > 1:
...
def get_lastfm(conf: config.Configuration) -> pd.DataFrame:
    dataset_dir = conf.assets_root_dir / "lastfm" / "data"
...
def get_movielens(conf: config.Configuration) -> pd.DataFrame:
    dataset_dir = conf.assets_root_dir / "movielens" / "data"
    ...
    topk_users = (
        bool_df.sum(axis=1)
        .sort_values(ascending=False)
        .index[: conf.movielens_topk_users]
    )
    ...
    topk_items = (
        bool_df.sum(axis=0)
        .sort_values(ascending=False)
        .index[: conf.movielens_topk_movies]
    )
